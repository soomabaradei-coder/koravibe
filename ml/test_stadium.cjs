...
(async()=>{
 ...
 const iphone=await browser.newPage({viewport:{width:390,height:844},userAgent:'Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 Version/18.0 Mobile/15E148 Safari/604.1'});
 await iphone.addInitScript(()=>Object.defineProperty(navigator,'vibrate',{value:undefined}));
 await iphone.goto(process.env.STADIUM_URL||'http://127.0.0.1:8000/stadium.html');
 assert.equal(await iphone.locator('#phone-title').textContent(),'iPhone: visual preview only');
 assert(await iphone.locator('#test-vibration').isDisabled());assert(await iphone.locator('#haptics').isDisabled());
 const audioResponse=await page.request.get(new URL('stadium/koravibe-test.mp3',page.url()).href);assert.equal(audioResponse.status(),200);assert((await audioResponse.body()).length>10000,'Real audio asset must be present');
